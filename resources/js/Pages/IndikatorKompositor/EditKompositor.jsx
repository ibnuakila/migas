...
export default function EditKompositor(props) {
    ...
    const { data, setData, put, errors, delete: destroy, processing } = useForm({
        ...
        status_kompositor_id: kompositor.data.status_kompositor_id || '',
        pics: defPics
    });
    const { flash } = props;
    ...
    const [sumberKompositor, setSumberKompositor] = useState(false);
    ...
    const handleSave = (e) => {
        e.preventDefault();
        //alert(sumberKompositor);
        if (sumberKompositor && (kompositor.data.sumber_kompositor_id !== data.sumber_kompositor_id)) {
            if (confirm("Anda telah merubah sumber kompositor dari " + kompositor.data.sumber_kompositor_id
                + ", menjadi " + data.sumber_kompositor_id
                + ". Hal ini bisa menghapus kompositor yang sudah diimport dalam Realisasi Laporan Capaian. Apakah Anda mau memprosesnya?")) {
    ...
    function handleChangeSumberKompositor(e) {
        if (e === 1) {
                                    ...
                                    <div className="sm:w-full md:w-full lg:w-full">
                                        <Select label="Sumber Kompositor" id="type-kompositor"
                                            ...
                                            {sumber_kompositor.map(({ id, nama_sumber_kompositor }) => (
                                                <Option value={String(id)} key={id}>{nama_sumber_kompositor}</Option>
                                            ))}
                                        </Select>
                                    </div>
                                        ...
                                        <div className="sm:w-full md:w-full lg:w-full">
                                            <Select label="Select Kompositor" id="indeks"
                                                ...
                                                {kompositors.map(({ id, nama_kompositor, jenis_kompositor_id }) => (
                                                    <Option value={id.toString()} key={id}>{nama_kompositor + " | " + jenis_kompositor_id}</Option>
                                                ))}
                                            </Select>
                                            {errors.kompositor_id && <div className="text-red-400 mt-1">{errors.kompositor_id}</div>}
                                        </div>) : (null)}
                                        ...
                                        <>
                                            <div className="sm:w-full md:w-full lg:w-full">
                                                <Select label="Parameter" id="parameter"
                                                    ...
                                                    value={parameter ? parameter.id.toString():null}
                                                >
                                            ...
                                            <div className="sm:w-full md:w-full lg:w-full">
                                                <Input label="Kalkulasi" variant="outlined" id="kalkulasi"
                                                    ...
                                                    defaultValue={parameter ? parameter.kalkulasi:null}
                                    ...
                                    <div className="sm:w-full md:w-full lg:w-full">
                                        <Select label="Status" id="jenis-kompositor"
                                            ...
                                            {status_kompositor.map(({ id, status_kompositor }) => (
                                                <Option value={id.toString()} key={id}>{status_kompositor}</Option>
                                            ))}
                                        </Select>
                                        {errors.parent_id && <div className="text-red-400 mt-1">{errors.parent_id}</div>}
                                    </div>
